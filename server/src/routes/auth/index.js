/**
 * Auth routes — register, verify-email, login, refresh, logout.
 *
...
router.post('/logout', async (req, res, next) => {
  try {
    ...
    if (!refreshToken) {
      // Treat missing token as already-logged-out — idempotent.
    ...
    try {
      ...
      // Expired or invalid token — the user is effectively already logged out.
      return res.status(200).json({ success: true, message: 'Logged out.' });
    }
